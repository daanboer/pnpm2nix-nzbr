{ lib, stdenv, nodejs, rsync, pkg-config, callPackage, writeText, runCommand
, ... }:

with builtins;
with lib;
with callPackage ./lockfile.nix { };
let
  nodePkg = nodejs;
  pkgConfigPkg = pkg-config;
in {
  mkPnpmPackage = { workspace ? null, components ? [ ]
    , src ? if (workspace != null && components != [ ]) then workspace else null
    , packageJSON ? src + "/package.json", componentPackageJSONs ? map (c: {
      name = "${c}/package.json";
      value = src + "/${c}/package.json";
    }) components, pnpmLockYaml ? src + "/pnpm-lock.yaml", pnpmWorkspaceYaml ?
      (if workspace == null then null else workspace + "/pnpm-workspace.yaml")
    ...
    , registry ? "https://registry.npmjs.org", script ? "build"
    , distDir ? "dist", distDirs ? (if workspace == null then
      [ distDir ]
    else
      (map (c: "${c}/dist") components)), distDirIsOut ? true
    , installNodeModules ? false, installPackageFiles ? false
    , installInPlace ? false, installEnv ? { }, buildEnv ? { }
    , noDevDependencies ? false, extraNodeModuleSources ? [ ]
    , copyPnpmStore ? true, copyNodeModules ? false, extraBuildInputs ? [ ]
    , nodejs ? nodePkg, pnpm ? nodejs.pkgs.pnpm, pkg-config ? pkgConfigPkg, ...
    }@attrs:
    let
      ...
      nativeBuildInputs = [ nodejs pnpm pkg-config ] ++ extraBuildInputs
        ++ (optional copyNodeModules rsync);
      copyLink = if copyNodeModules then "rsync -a --chmod=u+w" else "ln -s";
      rsyncSlash = optionalString copyNodeModules "/";
      packageFilesWithoutLockfile = [{
        name = "package.json";
        value = packageJSON;
      }] ++ componentPackageJSONs ++ computedNodeModuleSources;
      computedNodeModuleSources = (if pnpmWorkspaceYaml == null then
        [ ]
      else [{
        name = "pnpm-workspace.yaml";
        value = pnpmWorkspaceYaml;
      ...
      in distDirs ++ optionals installNodeModules nodeModulesDirs
      ++ optionals installPackageFiles packageFileNames;
      nodeModulesDirs = if isWorkspace then
        [ "node_modules" ] ++ (map (c: "${c}/node_modules") components)
      else
        [ "node_modules" ];
      filterString = concatStringsSep " "
        ([ "--recursive" "--stream" ] ++ map (c: "--filter ./${c}") components)
        + " ";
      buildScripts = ''
      ...
      computedDistDirIsOut = length computedDistFiles == 1 && distDirIsOut
        && !isWorkspace;
    in stdenv.mkDerivation (recursiveUpdate (rec {
      ...
      postUnpack = ''
        ...
        ''}
        ${forEachComponent
        (component: ''mkdir -p "${component}/node_modules"'')}
      '';
      ...
      in {
        ...
        patchedLockfileYaml =
          writeText "pnpm-lock.yaml" (toJSON passthru.patchedLockfile);
        ...
        pnpmStore = runCommand "${name}-pnpm-store" {
          ...
          pnpm store add ${
            concatStringsSep " " (unique processResult.dependencyTarballs)
          }
        '';
        ...
        nodeModules = stdenv.mkDerivation {
          ...
          unpackPhase = concatStringsSep "\n"
            ([ # components is an empty list for non workspace builds
              ...
              let
                nv = if isAttrs v then
                  v
                else {
                  name = ".";
                  value = v;
                };
              in ''cp -vr "${nv.value}" "${nv.name}"'') ([{
                name = "pnpm-lock.yaml";
                value = passthru.patchedLockfileYaml;
              }] ++ packageFilesWithoutLockfile));
          ...
          buildPhase = ''
            ...
            ${
              if !copyPnpmStore then "ln -s" else "cp -RL"
            ...
            pnpm install --stream ${
              optionalString noDevDependencies "--prod "
